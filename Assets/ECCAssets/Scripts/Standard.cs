...
namespace EndlessCarChase
{
    ...
    public class Standard : MonoBehaviour
    {
        ...
        //    if (moveForward != 0)
        //    {
        //        speed += 0.010f * moveForward; //Если игрок нажал вверх или вниз
        //        rotateSpeed = 200f;
        //        driftAngle = 50f;
        //    }
        ...
        void Update()
        {
            float moveSide = joystick.Horizontal;
            ...
            if (gameController.playerObject != this)
            {
                ...
                if (avoidObstacles == true && Physics.Raycast(rayRight, out hit, detectDistance) && (hit.transform.GetComponent<ECCObstacle>() || (hit.transform.GetComponent<Standard>() && gameController.playerObject != this)))
                {
                ...
                else if (avoidObstacles == true && Physics.Raycast(rayLeft, out hit, detectDistance) && (hit.transform.GetComponent<ECCObstacle>() || (hit.transform.GetComponent<Standard>() && gameController.playerObject != this))) // Otherwise, if we detect an obstacle on our left side, swerve to the right
                {
                    // Change the emission color of the obstacle to indicate that the car detected it
                    //if (hit.transform.GetComponent<MeshRenderer>()) hit.transform.GetComponent<MeshRenderer>().material.SetColor("_EmissionColor", Color.red);
            ...
            void OnTriggerStay(Collider other)
        {
            ...
            if (hurtDelayCount <= 0 && other.GetComponent<Standard>())
            {
